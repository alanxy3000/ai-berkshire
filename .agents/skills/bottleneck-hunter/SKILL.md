...
description: "AI Berkshire skill: 供应链瓶颈猎手：AI驱动的全球产业链瓶颈套利."
---
...
**台股标的取数**：供应链扫描常命中台股（4位数字代码，如 2330、3661），其行情/PER/市值/月营收统一用 `python3 tools/twstock_data.py`（FinMind 数据源，用法见 `.agents/skills/financial-data/SKILL.md` 台股章节）。台股每月10日前强制披露上月营收，月营收同比是验证瓶颈"量价齐升"最快的公开信号。
...
9. **遵循GEMINI.md客观性原则** — 不预设看多，先数据后结论
