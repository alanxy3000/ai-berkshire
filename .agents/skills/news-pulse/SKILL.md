...
使用 `invoke_subagent` 并行启动4个异动归因 Agent：
...
使用 `invoke_subagent` 一次性并行调用 4 个子Agent。每个 Agent 配置：
...
5. 严格区分"事实"与"推测"，遵循 GEMINI.md 客观性原则
...
7. **遵循 `GEMINI.md` 客观性原则**——所有判断附数据来源，区分事实与观点
