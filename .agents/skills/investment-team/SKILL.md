...
description: "AI Berkshire skill: 投研团队：四角色并行分析框架."
---

# 投研团队：四角色并行分析框架

对 $ARGUMENTS 进行团队化投资研究分析。使用 invoke_subagent 创建真正的多Agent并行投研团队。
...
通过 Antigravity CLI 的 `invoke_subagent` 工具并行启动4个子Agent：
...
使用 `invoke_subagent` 工具一次性并行启动4个子Agent：
...
- **财务数据必须来自两个独立来源**，按 `.agents/skills/financial-data/SKILL.md` 规范执行（美股：macrotrends+stockanalysis；港股：aastocks+macrotrends；A股：东方财富+巨潮资讯；台股：FinMind `tools/twstock_data.py`+Goodinfo），两源误差>1%须标记
- 确保数据准确，关键数据标注来源
- 分析要深入，不流于表面
...
全部报告收到后，主Agent对4个子Agent的结论进行汇总。
...
# Step 2 — 对清单每项从可靠信源取数（参见 .agents/skills/financial-data/SKILL.md）
