...
- **兼容性适配**：Antigravity CLI 新版本带来的适配问题。
...
2. 描述里写清楚：改了什么、为什么改、怎么验证的。涉及 skill 的改动，请附上你在 Antigravity CLI 里实际运行的效果。
3. 中文或英文均可。
...
- **Compatibility fixes** for new versions of Antigravity CLI.
...
- **New skill proposals** — open an issue first to discuss the research scenario it addresses and how it differs from the existing skills. Large unsolicited feature PRs may be closed to avoid wasted effort on both sides.
...
2. Describe what changed, why, and how you verified it. For skill changes, include results from an actual run in Antigravity CLI.
